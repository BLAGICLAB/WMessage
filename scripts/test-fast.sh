#!/usr/bin/env bash
# Pre-commit fast gate: format check + minimal compile check + pytest sanity + vitest.
...
echo "[2/4] cargo check (compile-only, 不跑测试)"
cargo check --manifest-path src-tauri/Cargo.toml --quiet

echo "[3/4] pytest collect-only (sanity, 不跑断言)"
python3 -m pytest tests-audit/audit_pre_step_pre_execute.py --collect-only -q

echo "[4/4] vitest run (前端 unit 测试)"
