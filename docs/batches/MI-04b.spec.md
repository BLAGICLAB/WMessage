...
- 预估 diff: 1 file / +26/-3 lines（B 类：插入守卫等待块；OCR r1 critical 采纳后 + 作用域限制块 + 等待上限，budget 校正两次：14→26（OCR critical 采纳）→ +33/-10（rustfmt 嵌套重排实测 +31/-8；超 SOP 一次额度，登记 META follow-up））
