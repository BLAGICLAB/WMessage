...
**`intents` 与动态路由（2026-08-19 老板拍板）**：
- 路由表不再有硬编码静态表：**路由条目 = 已安装技能的 `intents` 声明**。安装技能即生成其路由，卸载即移除；`enabled: false` 或未声明 `intents` 的技能不产生路由。
- 重建时机：app 启动一次 + `skills_import` / `skills_delete` 成功后各重建一次。
- 每条 intent 是**正则模式**，匹配用户消息全文（含 `[附件文件]` 块内嵌的附件路径）；大小写敏感由模式内联 `(?i)` 控制，多行匹配用 `(?is)`。
- **写法约束**：模式内含逗号（如量词 `{0,15}`）必须用上面的多行列表形式——单行 `["a","b"]` / 逗号分隔形式会把量词切碎。
- 附件上下文示例（消息带 .doc/.docx 附件 + 润色/修订关键词）：
