...
- [x] **NEW-C-4** `bot_model_loop/bot_slash StopGuard` — `/stop` 不覆盖在途 Python 子进程（只能等 60-120s 超时）
- [x] **NEW-C-5** `bot_py.rs:245/253` — 输出 >64KB → reader 关管道 → 子进程被 SIGPIPE 静默杀，`exit_code=None` 无说明
- [x] **NEW-C-6** `audit.rs:82 + bot_py.rs:849` — `write_event` kv 值 + `doc_extract` path 审计行不剥换行（与 P2-11 同族不同 sink）
- [x] **NEW-C-7** `bot_py.rs:1043` — `trim_end_matches(".{ext}")` 大小写敏感 → "周报.DOCX.docx"
...
- [x] **E4** `src/components/TaskCardContent.tsx:92-95` — Escape 取消后 blur 仍提交草稿
...
- [x] **NEW-D-2** `profile.rs:247-251` — `set_avatar` save 失败回滚 `remove_file(dest)` 在同扩展名覆盖场景误删在役头像（P2-18 未覆盖的反向回退）
