...
## 0. 执行日志
- [2026-09-26 11:19 CST] **DEC-REG**（9 项拍板决策收官登记，docs-only）：代码批 DEC-1（9ff9d1a，family decision-fixes-r1）落地 ① C4-v2（PANEL_H_MIN 800→400，区间 400–900，默认 560 不动——storage.test 新增默认 560 合法性回归钉）+ ⑤ S20（py setrlimit 逐资源探测降级：子进程探测+OnceLock 缓存，未生效/探测失败不设限额且 audit 每运行留痕，**探测失败≠可用**；实测裸 macOS AS 限额旧实现下从未生效、CPU 照设；OCR r1 1 medium 全采纳=探测值改生产上限）。登记项：② C2a-Q1 **永久 wontfix**（widget 保留拖拽/缩放为产品特性；翻案唯一路径=widget 只读化重构，届时 capability 拆分同批）；③ C2a-2 维持 defer（便携模式真实报障触发，PHASE6-T 账本行已注）；④ DB-02b 维持 wontfix-pending-design-decision（下方行已注确认）；⑥ Phase 3/4 翻案通道维持被动（运行时实证复现才提 issue，PHASE3/PHASE4 已注）；⑦ vendor tiny_http 维持 wontfix+触发器（PHASE4 已注，备选「对比上游 diff」被否）；⑧ HANDOFF v1 入库归档（6418b0c）；⑨ 月度 tests-audit 核对 cron 挂载（automation-bbbd0371，每月 1 号 09:00）+ 基线 docs/TESTS-AUDIT-MONTHLY-BASELINE.md（38 passed/1 skipped，报警落 docs/TESTS-AUDIT-MONTHLY-<date>.md + 系统通知）。配套：.mimosa/ gitignore（ae53553）。**fix-plan 真正收官，无遗留待办。**
- [2026-09-26 03:00 CST] **P6-1（Phase 6 门禁恢复，commit f2db879）：6 条 xfail 断言对齐现实全部转正（BOT glob 补 bot/config 子树 ×2 / start_skill_with_audit 形态 ×1 / 锚定 execute_tool_impl ×3）；resolve_entry 收紧（尾段唯一匹配，歧义响亮失败）暴露文档树 8 处裸条目已补全路径；test-all strict 零 xfail 门禁 + test-fast collect 扩 4 文件；HOOK-AUDIT-01 结案（always_allow 实测通过，矛盾消解）。pytest 38 passed / 0 xfailed。D2: files=6(+69/-39) asserts=0→0 tests=audit 38 绿 + test-all 1147 全绿。**Phase 6 全清。**
...
- C5-DB-02b：事务约定一致性 / 设计债（tasks.rs:439）→ **wontfix-pending-design-decision**，触发条件 = 未来引入 cascade/soft-delete 多语句 delete 时重审（2026-09-26 拍板确认维持：现状单语句 DELETE 本身原子，不预防性包事务）
