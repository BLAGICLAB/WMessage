...
- C5-BT-07：state machine / 乐观并发不一致（bot_artifacts.rs:113 + bot_skills/state.rs:164 + bot_skills/scheduler.rs:149），3 条 → **拆批**：:149 **已修（BT-07a，commit 6bd43c0，Drop 兜底）**；:113 疑似 FP（finding 称 db_upsert 不比 expected_updated_at，与 tasks.rs:185 (i) 决策核验矛盾，下批先核）；:164 全局清理 vs 按会话 = 语义方向，**B 类候选（攒批第 11 项）**
