...
- **#1 的 SIGTERM 来源：待确认（不写进 SOP 当既成事实）**。
  - **已确认事实**：`faint-wi` 于 17:12:51 收到 SIGTERM（harness 报告 `Exec failed (…, signal SIGTERM)`）；
    该轮耗时（~11.7min）> 我当时设的调用侧 `timeoutSeconds`（**600s**）。
  - **无法确认**：SIGTERM 由「调用侧 `timeoutSeconds` 到点」发出，还是由「harness 子进程清理
    hard deadline」发出（后者可解释 11.7 > 10min 的溢出）。**两者均为调用侧**；
    **无系统 reaper 证据**。
  - → 修复方案须对**两种调用侧来源**都成立（见下第 2 条）。
...
2. **待确认项的分支**：若最终判定 SIGTERM 来源为「harness hard deadline」（而非 `timeoutSeconds`）→
   调参数**不足**，需另加**防收割**（如 harness 侧进程组保护 / 显式产物心跳）。该分支同样**只写方案，下一批实现**。
