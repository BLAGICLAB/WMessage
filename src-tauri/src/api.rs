//! 本地 HTTP API：仅监听 127.0.0.1 的微服务（外部机器人接口）。
//!
//! 安全约束：
//! - 只绑定回环地址（127.0.0.1），不监听 0.0.0.0 / 公网
//! - 除健康检查外全部端点要求 `Authorization: Bearer <token>` 鉴权
//! - 不提供任何文件读取/目录遍历端点（filePath 只是任务数据字段）
//! - 请求体上限 1MB，每分钟 120 请求限流，无 CORS（无浏览器客户端）
//!
//! 端点：
...
//! 开关持久化：启动成功写 api-enabled.flag（api_stop 删除），App 下次启动自动恢复。
...
impl EventHub {
    ...
    /// 编号、入历史、广播给所有在线客户端
    fn broadcast(&self, event: serde_json::Value) {
    ...
    fn replay(&self, since: u64) -> Vec<String> {
...
/// 开关标志文件：api_start 成功写、api_stop 删，App 启动时存在则自动恢复 API
fn enabled_flag_path(app: &AppHandle) -> std::path::PathBuf {
...
fn write_enabled_flag(app: &AppHandle) {
    let dir = db::data_dir(app);
...
fn clear_enabled_flag(app: &AppHandle) {
